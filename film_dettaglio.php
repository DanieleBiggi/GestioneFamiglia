...
$stmtFilmPiattaforme = $conn->prepare("SELECT * FROM film2piattaforme WHERE id_film=?");
$stmtFilmPiattaforme->bind_param('i', $id);
$stmtFilmPiattaforme->execute();
$filmPiattaforme = [];
$resPiattaforme = $stmtFilmPiattaforme->get_result();
$data_aggiornamento_piattaforma = "";
while ($p = $resPiattaforme->fetch_assoc()) {
    $filmPiattaforme[] = (int)$p['id_piattaforma'];
    $data_aggiornamento_piattaforma = implode("/",array_reverse(explode("-",$p['indicata_il'])));
}
...
<div class="container text-white">
  ...
  <form method="post" class="bg-dark text-white p-3 rounded mb-4">
    ...
    <div class="mb-3">
      ...
      <div class="mb-3">
        <label class="form-label">Piattaforme streaming<?= $data_aggiornamento_piattaforma!=""?" (Aggiornato al ".$data_aggiornamento_piattaforma.")":"" ?></label>
