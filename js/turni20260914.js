document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnGoogle').addEventListener('click', async ()=>{
    ...
    try {
      const response = await fetch('ajax/turni_sync_google.php', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
      ...
      if (rawBody.trim()) {
        try {
          ...
          const preview = rawBody.trim().slice(0, 1500);
          throw new Error(
      ...
      if (!response.ok) {
        const serverMessage = data?.message || data?.error || rawBody.trim() || response.statusText;
        throw new Error(`HTTP ${response.status}: ${serverMessage}`);
      }

      if (data?.success === false) {
        throw new Error(data.message || data.error || 'Il server ha segnalato un errore di sincronizzazione.');
      }
      ...
      loadingModal && loadingModal.hide();
    }
  });
